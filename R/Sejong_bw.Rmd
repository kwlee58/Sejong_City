...
g3 <- g2
g4 <- g3 + geom_segment(aes(x = Before, y = reorder(Project, After), xend = After, yend = Project, alpha = Division), arrow = arrow(length = unit(0.1, "inches"), ends = "last", type = "closed"), lineend = "round")
g5 <- g4 + xlab("위원회점수와 부서점수") + ylab("포괄보조사업") + labs(alpha = "분과") + theme(legend.position = c(.15,.8))
...
h2 <- h1 + xlim(40, 100)
h3 <- h2 + geom_segment(aes(x = Before, y = Project.f, xend = After, yend = Project.f, alpha = Division), arrow = arrow(length = unit(0.1, "inches"), ends = "last", type = "closed"))
h4 <- h3 + xlab("위원회점수와 부서점수") + ylab("포괄보조사업") + labs(alpha = "분과") + theme(legend.position = c(.15,.85))
